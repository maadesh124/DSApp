...
class NApplication extends Model {
  ...
  double learnerAge;
   ...
   String napplicationNumber;
   String courseId; // Required
   String schoolId; // Can be null if service napplication
   String serviceId; // Can be null if course napplication
   String courseName; // Added
   String learnerName; // Added
   String serviceName;

  NApplication({super.collectionType=Model.APPLICATION,
    dateApplied,
    this.learnerAddress='',
    this.learnerAge=20,
    this.isMale=false,
    this.learnerObjectId='',
    this.napplicationNumber='',
  ...
  napplicationNumber= map['napplicationNumber'] as String;
  ...
  Error.throwError('Error in Napplication fromMap');
}
  ...
  Map<String, dynamic> toMap() {
    return {
      ...
      'napplicationNumber': napplicationNumber,
      'courseId': courseId,
      'schoolId': schoolId,
      'serviceId': serviceId,
...
//    where('napplicationNumber',isEqualTo: napplicationNumber).get().then((value) => value.docs.first);
...
//   }
  ...
  // static Future<NApplication> createCourse(Course course,Learner learner,String learnerObjectId) async
  // {
    
  //   NApplication napplication=NApplication(learnerAddress: learner.address, 
  //   courseName: course.name,
  //   learnerName: learner.name,
  //   learnerAge: learner.age+0.0,
  //    isMale: (learner.gender=='Male'),
  //     learnerObjectId: learnerObjectId,
  //      napplicationNumber: course.dsObjectId+course.applicationObjectIds.length.toString(), 
  //      courseId: course.courseId,
  //       schoolId: course.dsObjectId);

  //  final appref= await FirebaseFirestore.instance.collection(DataBase.APPLICATION_COLLECTION).
  //  add(napplication.toMap());
